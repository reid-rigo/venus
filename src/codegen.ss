;;; Venus Codegen for Chez Scheme
;;; Generates Scheme source code from Venus AST (association lists).

(define codegen-indent 0)

(define (cg-emit port line)
  (if (= (string-length line) 0)
      (newline port)
      (begin
        (do ((i 0 (+ i 1))) ((>= i codegen-indent))
          (display "  " port))
        (display line port)
        (newline port))))

(define (cg-indent! delta)
  (set! codegen-indent (+ codegen-indent delta)))

(define (ast-type node)
  (let ((p (assq 'type node)))
    (if p (cdr p) #f)))

(define (ast-ref node key)
  (let ((p (assq key node)))
    (if p (cdr p) #f)))

(define (string-replace str from to)
  (let* ((len (string-length str))
         (flen (string-length from))
         (buf (open-output-string)))
    (let loop ((i 0))
      (if (>= i len)
          (get-output-string buf)
          (if (and (<= (+ i flen) len)
                   (string=? (substring str i (+ i flen)) from))
              (begin (display to buf) (loop (+ i flen)))
              (begin (write-char (string-ref str i) buf) (loop (+ i 1))))))))

(define (any pred lst)
  (cond
    ((null? lst) #f)
    ((pred (car lst)) #t)
    (else (any pred (cdr lst)))))

(define (string-join lst sep)
  (if (null? lst)
      ""
      (let loop ((rest (cdr lst)) (result (car lst)))
        (if (null? rest)
            result
            (loop (cdr rest) (string-append result sep (car rest)))))))

(define match-counter 0)

(define (ident->scheme name)
  (string-replace name "." "-"))

(define (cg-process-string val)
  (let ((len (string-length val)))
    (if (and (>= len 6)
             (string=? (substring val 0 3) "\"\"\"")
             (string=? (substring val (- len 3) len) "\"\"\""))
        (string-append "\""
                       (string-replace
                        (string-replace
                         (substring val 3 (- len 3))
                         "\\" "\\\\")
                        "\"" "\\\"")
                       "\"")
        val)))

(define (cg-emit-fn-body port body)
  (let loop ((stmts body))
    (if (null? stmts)
        'done
        (let ((stmt (car stmts))
              (rest (cdr stmts)))
          (if (eq? (ast-type stmt) 'let)
              (let ((names (ast-ref stmt 'names))
                    (values (ast-ref stmt 'values)))
                (if (null? values)
                    (begin
                      (cg-emit port (string-append "(let ((" (car names) " #f))"))
                      (cg-indent! 1)
                      (loop rest)
                      (cg-indent! -1)
                      (cg-emit port ")"))
                    (begin
                      (cg-emit port (string-append "(let ((" (car names) " "
                                             (cg-emit-expr port (car values)) "))"))
                      (cg-indent! 1)
                      (loop rest)
                      (cg-indent! -1)
                      (cg-emit port ")"))))
              (let ((line (cg-emit-expr port stmt)))
                (when (not (string=? line ""))
                  (cg-emit port line))
                (loop rest)))))))

(define (cg-emit-expr port node)
  (let ((type (ast-type node)))
    (cond
      ((eq? type 'number)
       (ast-ref node 'value))

      ((eq? type 'string)
       (cg-process-string (ast-ref node 'value)))

      ((eq? type 'interp-string)
       (string-append "(string-append "
                       (string-join
                        (map (lambda (part)
                               (if (eq? (ast-type part) 'string)
                                   (cg-process-string (ast-ref part 'value))
                                   (cg-emit-expr port part)))
                             (ast-ref node 'parts))
                        " ")
                       ")"))

      ((eq? type 'nil)
       "#f")

      ((eq? type 'true)
       "#t")

      ((eq? type 'false)
       "#f")

      ((eq? type 'ident)
       (ident->scheme (ast-ref node 'name)))

      ((eq? type 'member)
       (string-append "(cdr (assq '" (ast-ref node 'field) " "
                      (cg-emit-expr port (ast-ref node 'object)) "))"))

      ((eq? type 'safe-member)
       (let ((obj (cg-emit-expr port (ast-ref node 'object)))
             (fields (ast-ref node 'fields)))
         (string-append "(and " obj
                        " (cdr (assq '" (car fields) " " obj ")))")))

      ((eq? type 'binary)
       (let ((left (cg-emit-expr port (ast-ref node 'left)))
             (op (ast-ref node 'op))
             (right (cg-emit-expr port (ast-ref node 'right))))
         (cond
           ((string=? op "==")
            (string-append "(equal? " left " " right ")"))
           ((string=? op "!=")
            (string-append "(not (equal? " left " " right "))"))
           ((string=? op "and")
            (string-append "(and " left " " right ")"))
           ((string=? op "or")
            (string-append "(or " left " " right ")"))
           (else
            (string-append "(" op " " left " " right ")")))))

      ((eq? type 'unary)
       (let ((operand (cg-emit-expr port (ast-ref node 'operand)))
             (op (ast-ref node 'op)))
         (if (string=? op "!")
             (string-append "(not " operand ")")
             (string-append "(- " operand ")"))))

      ((eq? type 'call)
       (let* ((callee-raw (ast-ref node 'callee))
              (callee
               (cond
                 ((string? callee-raw)
                  callee-raw)
                 ((eq? (ast-type callee-raw) 'member)
                  (string-append (cg-emit-expr port (ast-ref callee-raw 'object))
                                 "-" (ast-ref callee-raw 'field)))
                 ((eq? (ast-type callee-raw) 'lambda)
                  (string-append "(" (cg-emit-expr port callee-raw) ")"))
                 (else
                  (cg-emit-expr port callee-raw))))
              (args (ast-ref node 'args))
              (arg-strs (map (lambda (a) (cg-emit-expr port a)) args)))
         (let ((args-joined (string-join arg-strs " ")))
           (if (string=? callee "print")
               (string-append "(vs-print " args-joined ")")
               (string-append "(" callee (if (string=? args-joined "") "" (string-append " " args-joined)) ")")))))

      ((eq? type 'let)
       (let ((names (ast-ref node 'names))
             (values (ast-ref node 'values)))
         (if (null? values)
             (string-append "(define " (car names) " #f)")
             (string-append "(define " (car names)
                            " " (cg-emit-expr port (car values)) ")"))))

      ((eq? type 'lambda)
       (let ((saved-indent codegen-indent))
         (let ((result
                (with-output-to-string
                  (lambda ()
                    (let ((port (current-output-port)))
                      (cg-emit port (string-append "(lambda ("
                                                    (string-join (ast-ref node 'params) " ")
                                                    ")"))
                      (cg-indent! 1)
                      (cg-emit-fn-body port (ast-ref node 'body))
                      (cg-indent! -1)
                      (cg-emit port ")"))))))
           (set! codegen-indent saved-indent)
           result)))

      ((eq? type 'fn)
       (let* ((params (map (lambda (p)
                             (or (ast-ref p 'name) (ast-ref p 'value)))
                           (ast-ref node 'params)))
              (params-str (if (null? params) "" (string-append " " (string-join params " ")))))
         (cg-emit port (string-append "(define ("
                                       (ast-ref node 'name)
                                       params-str ")"))
         (cg-indent! 1)
         (cg-emit-fn-body port (ast-ref node 'body))
         (cg-indent! -1)
         (cg-emit port ")")
         ""))

      ((eq? type 'list)
       (let ((vals (map (lambda (v) (cg-emit-expr port v)) (ast-ref node 'values))))
         (string-append "(list " (string-join vals " ") ")")))

      ((eq? type 'table)
       (let ((fields (map (lambda (f)
                            (string-append "(cons '"
                                           (ast-ref f 'key) " "
                                           (cg-emit-expr port (ast-ref f 'value)) ")"))
                          (ast-ref node 'fields))))
         (string-append "(list " (string-join fields " ") ")")))

      ((eq? type 'if)
       (let ((saved-indent codegen-indent))
         (let ((result
                (with-output-to-string
                  (lambda ()
                    (let ((p (current-output-port)))
                      (cg-emit p (string-append "(if " (cg-emit-expr p (ast-ref node 'condition))))
                      (cg-indent! 1)
                      (cg-emit-fn-body p (ast-ref node 'body))
                      (cg-indent! -1)
                      (for-each
                       (lambda (ei)
                         (cg-emit p (string-append "(if " (cg-emit-expr p (ast-ref ei 'condition))))
                         (cg-indent! 1)
                         (cg-emit-fn-body p (ast-ref ei 'body))
                         (cg-indent! -1))
                       (ast-ref node 'else-ifs))
                      (when (ast-ref node 'else-body)
                        (cg-emit p "(else")
                        (cg-indent! 1)
                        (cg-emit-fn-body p (ast-ref node 'else-body))
                        (cg-indent! -1)
                        (cg-emit p ")"))
                      (do ((i 0 (+ i 1)))
                          ((>= i (+ 1 (length (ast-ref node 'else-ifs)))))
                        (cg-emit p ")")))))))
           (set! codegen-indent saved-indent)
           result)))

      ((eq? type 'match)
       (set! match-counter (+ match-counter 1))
       (let ((tmp (string-append "_m_" (number->string match-counter)))
             (value-code (cg-emit-expr port (ast-ref node 'value))))
         (let ((saved-indent codegen-indent))
           (let ((result
                  (with-output-to-string
                    (lambda ()
                      (let ((p (current-output-port)))
                        (display "(let (((" p) (display tmp p) (display " " p) (display value-code p) (display ")))" p) (newline p)
                        (set! codegen-indent (+ saved-indent 1))
                        (let ((has-if #f))
                          (for-each
                           (lambda (arm)
                             (let* ((pat (ast-ref arm 'pattern))
                                    (body-code (cg-emit-expr p (ast-ref arm 'body))))
                               (cond
                                 ((eq? (ast-type pat) 'wildcard)
                                  (if has-if
                                      (cg-emit p "(else")
                                      (begin (cg-emit p "(else") (set! has-if #t)))
                                  (cg-indent! 1)
                                  (cg-emit p body-code)
                                  (cg-indent! -1)
                                  (cg-emit p ")"))
                                 ((eq? (ast-type pat) 'ident)
                                  (cg-emit p (string-append "(let (((" (ast-ref pat 'name) " " tmp ")))"))
                                  (cg-indent! 1)
                                  (cg-emit p body-code)
                                  (cg-indent! -1)
                                  (cg-emit p ")"))
                                 (else
                                  (let ((pat-code (cg-emit-expr p pat)))
                                    (let ((ind (if has-if "  " "")))
                                      (cg-emit p (string-append ind "(if (equal? " tmp " " pat-code ")"))
                                      (set! has-if #t)
                                      (cg-indent! 1)
                                      (cg-emit p body-code)
                                      (cg-indent! -1)
                                      (cg-emit p ")")))))))
                           (ast-ref node 'arms))
                          (when (not has-if)
                            (cg-emit p "(else")
                            (cg-indent! 1)
                            (cg-emit p "#f")
                            (cg-indent! -1)
                            (cg-emit p ")")))
                        (display ")" p) (newline p))))))
             (set! codegen-indent saved-indent)
             result))))

      ((eq? type 'placeholder)
       (error 'codegen "placeholder outside pipeline context"))

      ((eq? type 'program)
       (cg-emit port "(import (chezscheme))")
       (cg-emit port "")
       (cg-emit port "(define (vs-print . args)")
       (cg-indent! 1)
       (cg-emit port "(unless (null? args)")
       (cg-indent! 1)
       (cg-emit port "(display (car args))")
       (cg-emit port "(for-each (lambda (x) (display \" \") (display x)) (cdr args))")
       (cg-indent! -1)
       (cg-emit port ")")
       (cg-emit port "(newline)")
       (cg-indent! -1)
       (cg-emit port ")")
       (cg-emit port "")
       (for-each
        (lambda (stmt)
          (let ((line (cg-emit-expr port stmt)))
            (when (not (string=? line ""))
              (cg-emit port line))))
        (ast-ref node 'body))
       "")

      (else
       (error 'codegen (format "Unknown node type: ~s" type))))))

(define (codegen ast)
  (set! codegen-indent 0)
  (set! match-counter 0)
  (let ((port (open-output-string)))
    (cg-emit-expr port ast)
    (let ((result (get-output-string port)))
      (if (and (> (string-length result) 0)
               (char=? (string-ref result (- (string-length result) 1)) #\newline))
          (substring result 0 (- (string-length result) 1))
          result))))
