       ...
       ((eq? type 'member)
        ...
        ((eq? type 'safe-member)
         (let ((obj (cg-emit-expr port (ast-ref node 'object)))
            ...
            (string-append "(if (let ((v " obj ")) (and (not (eq? v #f)) (not (eq? v (quote nil))))) (venus-ref " obj " \"" (car fields) "\") venus-nil)")))
                  ...
                  (with-output-to-string
                    (lambda ()
                      (let ((p (current-output-port)))
                        (define (emit-truthy condition)
                          (let ((c (cg-emit-expr p condition)))
                            (cg-emit p (string-append "((let ((v " c ")) (and (not (eq? v #f)) (not (eq? v (quote nil)))))"))))
                        (cg-emit p "(cond")
                        (cg-indent! 1)
                        (emit-truthy (ast-ref node 'condition))
                        (cg-indent! 1)
                        ...
                        (for-each
                         (lambda (ei)
                           (emit-truthy (ast-ref ei 'condition))
                           (cg-indent! 1)
