    ...
    (cond
      ...
      ((eq? type 'overloaded_fn)
       (let* ((name (ast-ref node 'name))
              ...
              (simple (and (= (length overloads) 1)
                           (not (cg-has-literal-params (car overloads))))))
         (if simple
             (let* ((fn (car overloads))
                    (params (map (lambda (p) (or (ast-ref p 'name) (ast-ref p 'value))
                                             ) (ast-ref fn 'params)))
                    (pstr (if (null? params) "" (string-append " " (string-join params " ")))))
               (cg-emit port (string-append "(define (" name pstr ")"))
               (cg-indent! 1)
               (cg-emit-fn-body port (ast-ref fn 'body))
               (cg-indent! -1)
               (cg-emit port ")")
               "")
             (let ((pstr (string-append "(" name " . args)")))
               (cg-emit port (string-append "(define " pstr))
               (cg-indent! 1)
               ...
               (for-each
                (lambda (ol)
                  (let ((conds (filter (lambda (s) (not (string=? s "")))
                                       (map (lambda (p)
                                              (if (eq? (ast-ref p 'is-literal) #t)
                                                   (string-append "(equal? " (ast-ref p 'value) " n)")
                                                  ""))
                                            (ast-ref ol 'params)))))
(if (null? conds)
    (cg-emit port "(else")
    (cg-emit port (string-append "(" (if (= (length conds) 1)
                                          (car conds)
                                          (string-append "(and " (string-join conds " ") ")")))))
                     (cg-indent! 1)
